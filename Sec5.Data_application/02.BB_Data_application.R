# ============================================================
# 02.BB_Data_application.R
# ------------------------------------------------------------
# Data application for the Binary Outcome + Binary Mediator (BC) case
# corresponding to Section 5.2 of the manuscript.
#
# Dataset:
#   - We use the student dataset bundled with the {mediation} package.
#     No external data files are required.
#
# Reference:
#   Tingley, D., Yamamoto, T., Hirose, K., Keele, L., & Imai, K. (2014).
#   "Mediation: R package for causal mediation analysis."
#   Journal of Statistical Software, 59, 1–38.
#
# This script performs:
#   1) Data preparation and modeling formulas
#   2) Point estimation of NDE, NIE, MP and bias-corrected variants
#   3) Bootstrap inference (percentile 95% CI) for all quantities
#
# Dependencies:
#   - Requires 00.basic_functions.R and 04.BB_functions.R
# ============================================================

source('./functions/00.basic_functions.R')
source('./functions/04.BB_functions.R')

# ------------------------------------------------------------
# 1) Load & Prepare Data
# ------------------------------------------------------------
# Variables kept:
#   Y (binary):  fight       - fight at school (1 = yes)
#   X (binary):  catholic    - catholic school (1 = yes)
#   M (binary):  attachment  - attachment to school (1 = like)
#   C : gender (0:Male, 1:Female), income (1–16, treated as continuous), pared (1 = Yes)
# ------------------------------------------------------------

data("student", package = "mediation")

modeling_data <- student %>%
  dplyr::select(fight, catholic, attachment, gender, income, pared)

# Ensure binary variables are factors
modeling_data$fight      <- as.factor(modeling_data$fight)
modeling_data$catholic   <- as.factor(modeling_data$catholic)
modeling_data$attachment <- as.factor(modeling_data$attachment)

# Confounders
# gender, pared: keep as factors (0/1)
modeling_data$gender <- as.factor(modeling_data$gender)
modeling_data$pared  <- as.factor(modeling_data$pared)

# income: numeric/ordered scale (kept numeric)


# ------------------------------------------------------------
# 2) Modeling Setup (BB case: Binary Y, Binary M)
# ------------------------------------------------------------
# Conditional effects at selected covariate combinations:
#   - gender: {0, 1}
#   - income: sample quantiles (Q1, median, Q3)
#   - pared : {0, 1}
# This yields a 2 (gender) × 3 (income) × 2 (pared) grid
# (a "2×2 table by gender" across pared levels with income at 3 anchors).
# ------------------------------------------------------------

med_formula <- as.formula("attachment ~ catholic + gender + income + pared")
out_formula <- as.formula("fight ~ catholic + attachment + catholic:attachment + gender + income + pared")

income_grid  <- c(summary(modeling_data$income)[2],  # Q1
                  summary(modeling_data$income)[3],  # median
                  summary(modeling_data$income)[5])  # Q3
pared_grid   <- c(0, 1)
gender_grid  <- c(0, 1)

u_combi <- expand.grid(gender = gender_grid, income = income_grid, pared = pared_grid)


# ------------------------------------------------------------
# 3) Point Estimation
# ------------------------------------------------------------
# For each covariate combination u, we compute:
#   - Ordinary: NDE, NIE, MP
#   - Bias-corrected: NDE_BC1/BC2, NIE_BC1/BC2
#   - MP_BC1 / MP_BC2 are computed as ratio-form BC with BC2-inserted NDE/NIE
#     (i.e., we take the “_insert_bc2” variants from BCA.bb for MP).
# ------------------------------------------------------------

# Fit mediator model (logistic) and outcome model (logistic)
Med <- glm(med_formula, family = binomial, data = modeling_data)
th  <- summary(Med)$coefficients[, 1]      # theta estimates
sig_theta <- vcov(Med)                      # Var-Cov(theta)

Outcome <- glm(out_formula, family = binomial, data = modeling_data)
beta <- summary(Outcome)$coefficients[, 1]  # beta estimates
# Reorder to (Intercept, X, M, X:M, U's) as used by BB functions
beta <- c(beta[1], beta[2], beta[3], beta[length(beta)], beta[4:(length(beta) - 1)])
sig_beta <- vcov(Outcome)                    # Var-Cov(beta)

# Joint covariance for (beta, theta); 'u' here provides the confounder design
conf <- modeling_data[, 4:(length(beta) - 1)]
COV  <- VAR.C(beta = beta, th = th, u = conf, sig.beta = sig_beta, sig.th = sig_theta)

# Container for point estimates across all (gender, income, pared)
Result_Ord <- data.frame(
  gender = 0, income = 0, pared = 0,
  NDE = 0, NIE = 0, MP = 0,
  NDE_BC1 = 0, NDE_BC2 = 0,
  NIE_BC1 = 0, NIE_BC2 = 0,
  MP_BC1 = 0, MP_BC2 = 0
)

for (i in 1:nrow(u_combi)) {
  u_unit <- u_combi[i, ]
  
  est <- CMA.bb(beta = beta, th = th, u = u_unit, x1 = 1, x2 = 0)
  bc  <- BCA.bb(beta = beta, th = th, u = u_unit, x1 = 1, x2 = 0, COV = COV)
  
  # Use the “insert_bc2” variants for MP, matching our write-up
  Result_Ord[i, ] <- unlist(c(
    u_unit$gender, u_unit$income, u_unit$pared,
    bc["NDE"], bc["NIE"], bc["MP"],
    bc["NDE_BC1"], bc["NDE_BC2"],
    bc["NIE_BC1"], bc["NIE_BC2"],
    bc["MP_BC1_insert_bc2"], bc["MP_BC2_insert_bc2"]
  ))
}

Result_Ord <- Result_Ord %>% arrange(gender, income, pared)


# ------------------------------------------------------------
# 4) Bootstrap Inference (percentile 95% CI)
# ------------------------------------------------------------
# We bootstrap the full procedure (refit Med/Outcome; recompute COV, CMA, BCA)
# for each covariate combination u. Returns percentile 95% CIs.
# ------------------------------------------------------------

n <- nrow(modeling_data)  # 9679
Result_boot <- data.frame()

for (i in 1:nrow(u_combi)) {
  u_unit <- u_combi[i, ]
  
  mediation_stat <- function(data, indices) {
    data_b <- data[indices, ]
    
    # Refit mediator model
    Med_b <- glm(med_formula, family = binomial, data = data_b)
    th_b  <- summary(Med_b)$coefficients[, 1]
    sig_theta_b <- vcov(Med_b)
    
    # Refit outcome model
    Outcome_b <- glm(out_formula, family = binomial, data = data_b)
    beta_b <- summary(Outcome_b)$coefficients[, 1]
    beta_b <- c(beta_b[1], beta_b[2], beta_b[3], beta_b[length(beta_b)], beta_b[4:(length(beta_b) - 1)])
    sig_beta_b <- vcov(Outcome_b)
    
    # Joint covariance
    conf_b <- data_b[, 4:(length(beta_b) - 1)]
    COV_b  <- VAR.C(beta = beta_b, th = th_b, u = conf_b,
                    sig.beta = sig_beta_b, sig.th = sig_theta_b)
    
    # Recompute ordinary & bias-corrected effects at u_unit
    est_b <- CMA.bb(beta = beta_b, th = th_b, u = u_unit, x1 = 1, x2 = 0)
    bc_b  <- BCA.bb(beta = beta_b, th = th_b, u = u_unit, x1 = 1, x2 = 0, COV = COV_b)
    
    # Return vector: (NDE, NIE, MP, NDE_BC1, NDE_BC2, NIE_BC1, NIE_BC2, MP_BC1_insert_bc2, MP_BC2_insert_bc2)
    return(unlist(c(est_b["NDE"], est_b["NIE"], est_b["MP"],
                    bc_b["NDE_BC1"], bc_b["NDE_BC2"],
                    bc_b["NIE_BC1"], bc_b["NIE_BC2"],
                    bc_b["MP_BC1_insert_bc2"], bc_b["MP_BC2_insert_bc2"])))
  }
  
  set.seed(i)
  boot_res <- boot(data = modeling_data, statistic = mediation_stat, R = 1000)
  
  # Percentile bootstrap 95% CIs
  ci_NDE     <- boot.ci(boot_res, index = 1, type = "perc")$perc[4:5]
  ci_NIE     <- boot.ci(boot_res, index = 2, type = "perc")$perc[4:5]
  ci_MP      <- boot.ci(boot_res, index = 3, type = "perc")$perc[4:5]
  ci_NDE_BC1 <- boot.ci(boot_res, index = 4, type = "perc")$perc[4:5]
  ci_NDE_BC2 <- boot.ci(boot_res, index = 5, type = "perc")$perc[4:5]
  ci_NIE_BC1 <- boot.ci(boot_res, index = 6, type = "perc")$perc[4:5]
  ci_NIE_BC2 <- boot.ci(boot_res, index = 7, type = "perc")$perc[4:5]
  ci_MP_BC1  <- boot.ci(boot_res, index = 8, type = "perc")$perc[4:5]
  ci_MP_BC2  <- boot.ci(boot_res, index = 9, type = "perc")$perc[4:5]
  
  # Collect point estimates (t0) and CIs
  temp <- data.frame(
    gender = u_unit$gender,
    income = u_unit$income,
    pared  = u_unit$pared,
    
    NDE       = boot_res$t0[1],
    NDE_CI_L  = ci_NDE[1],     NDE_CI_U  = ci_NDE[2],
    NIE       = boot_res$t0[2],
    NIE_CI_L  = ci_NIE[1],     NIE_CI_U  = ci_NIE[2],
    MP        = boot_res$t0[3],
    MP_CI_L   = ci_MP[1],      MP_CI_U   = ci_MP[2],
    
    NDE_BC1      = boot_res$t0[4],
    NDE_BC1_CI_L = ci_NDE_BC1[1], NDE_BC1_CI_U = ci_NDE_BC1[2],
    NDE_BC2      = boot_res$t0[5],
    NDE_BC2_CI_L = ci_NDE_BC2[1], NDE_BC2_CI_U = ci_NDE_BC2[2],
    
    NIE_BC1      = boot_res$t0[6],
    NIE_BC1_CI_L = ci_NIE_BC1[1], NIE_BC1_CI_U = ci_NIE_BC1[2],
    NIE_BC2      = boot_res$t0[7],
    NIE_BC2_CI_L = ci_NIE_BC2[1], NIE_BC2_CI_U = ci_NIE_BC2[2],
    
    MP_BC1      = boot_res$t0[8],
    MP_BC1_CI_L = ci_MP_BC1[1], MP_BC1_CI_U = ci_MP_BC1[2],
    MP_BC2      = boot_res$t0[9],
    MP_BC2_CI_L = ci_MP_BC2[1], MP_BC2_CI_U = ci_MP_BC2[2]
  )
  
  Result_boot <- rbind(Result_boot, temp)
  message(sprintf("%d / %d completed", i, nrow(u_combi)))
}

rownames(Result_boot) <- NULL
Result_boot <- Result_boot %>% arrange(gender, income, pared)

# ------------------------------------------------------------
# (Optional) Save outputs
# ------------------------------------------------------------
save(Result_boot,file="./Sec5.Data_application/Data_application_result/02.BB_application.RData")
