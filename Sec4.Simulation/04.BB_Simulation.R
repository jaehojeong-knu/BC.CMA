...
# 04.BB_Simulation.R
# ------------------------------------------------------------
# Simulation script for Binary Outcome and Binary Mediator case
# corresponding to Section 4.4 in the manuscript.
#
...
#
# Dependencies:
#   - Requires 00.basic_functions.R and 04.BB_functions.R
# ============================================================

source('./functions/00.basic_functions.R')
source('./functions/04.BB_functions.R')
...
th1   <- c(-1.00, 2.40, 0.05)
beta1 <- c(0.50, 0.30, 0.60, 0.45, 0.10)

th2   <- c(-1.00, 2.50, 0.05)
beta2 <- c(0.50, 0.70, 0.50, 0.20, 0.10)

th3   <- c(-1.00, 2.40, 0.05)
beta3 <- c(0.50, 0.90, 0.15, 0.30, 0.10)

theta_true <- cbind(th1, th2, th3)
beta_true  <- cbind(beta1, beta2, beta3)

# Compute "true" effects using CMA.bb (analytical target)
TrueNIE <- rep(0, 3)
TrueNDE <- rep(0, 3)
TrueMP  <- rep(0, 3)
...
for (m in 1:3) {
  ...
  th.T   <- theta_true[, i]
  beta.T <- beta_true[, i]
  True   <- CMA.bb(beta.T, th.T, u, x1 = x1, x2 = x2)
  TrueNIE[i] <- True$NIE
  TrueNDE[i] <- True$NDE
  TrueMP[i]  <- True$MP
}
true_value_BB <- cbind(TrueNDE, TrueNIE, TrueMP, t(beta_true), t(theta_true)); true_value_BB
...
for (i in c(1, 2, 3)) {
  for (N in c(100, 200, 300, 400, 600)) {
    ...
    for (m in 1:M) {
      ...
      # Data Generation
      # --------------------------------------------------------
      Dat <- Surv.sim.bb(N, beta = beta_sim, theta = theta_sim)
      ...
      # --------------------------------------------------------
      # Outcome Model (Logistic for Binary Outcome)
      # --------------------------------------------------------
      Outcome <- glm(out_formula, family = binomial, data = data)
      # Skip this iteration if coefficients contain NA
      ...
      # --------------------------------------------------------
      CASE.CMA <- BCA.bb(beta = beta, th = th, u = u, x1 = x1, x2 = x2, COV = COV)
    ...
    # --------------------------------------------------------
    assign(paste0("LIST_BB_i", i, "_N", N),
           data.frame(NDE      = CASE.NDE,
    ...
    write.csv(
      get(paste0("LIST_BB_i", i, "_N", N)),
      paste0(
        "./Sec4.Simulation/Simulation_data/04.BB_data/",
        paste0("LIST_BB_i", i, "_N", N), ".csv"
      ),
      row.names = FALSE
    )
  }
...
folder_path <- "./Sec4.Simulation/Simulation_data/04.BB_data/"
...
for (i in i_values) {
  for (N in N_values) {
    var_name  <- paste0("LIST_BB_i", i, "_N", N)
    file_path <- paste0(folder_path, var_name, ".csv")
    BB_data_list[[var_name]] <- read.csv(file_path)
  }
}
...
Simulation_summary_BB <- function(df, i, N) {
  
  #### NDE
  Est_NDE     <- mean(df$NDE, na.rm = TRUE)
  Var10_NDE   <- 10 * var(df$NDE, na.rm = TRUE)
  Bias_NDE    <- Est_NDE - true_value_BB[i, "TrueNDE"]
  Rbias100_NDE <- 100 * (Est_NDE - true_value_BB[i, "TrueNDE"]) / true_value_BB[i, "TrueNDE"]
  
  Est_NDE_BC1   <- mean(df$NDE_BC1, na.rm = TRUE)
  Var10_NDE_BC1 <- 10 * var(df$NDE_BC1, na.rm = TRUE)
  Bias_NDE_BC1  <- Est_NDE_BC1 - true_value_BB[i, "TrueNDE"]
  Rbias100_NDE_BC1 <- 100 * (Est_NDE_BC1 - true_value_BB[i, "TrueNDE"]) / true_value_BB[i, "TrueNDE"]
  
  Est_NDE_BC2   <- mean(df$NDE_BC2, na.rm = TRUE)
  Var10_NDE_BC2 <- 10 * var(df$NDE_BC2, na.rm = TRUE)
  Bias_NDE_BC2  <- Est_NDE_BC2 - true_value_BB[i, "TrueNDE"]
  Rbias100_NDE_BC2 <- 100 * (Est_NDE_BC2 - true_value_BB[i, "TrueNDE"]) / true_value_BB[i, "TrueNDE"]
  
  #### NIE
  Est_NIE     <- mean(df$NIE, na.rm = TRUE)
  Var10_NIE   <- 10 * var(df$NIE, na.rm = TRUE)
  Bias_NIE    <- Est_NIE - true_value_BB[i, "TrueNIE"]
  Rbias100_NIE <- 100 * (Est_NIE - true_value_BB[i, "TrueNIE"]) / true_value_BB[i, "TrueNIE"]
  
  Est_NIE_BC1   <- mean(df$NIE_BC1, na.rm = TRUE)
  Var10_NIE_BC1 <- 10 * var(df$NIE_BC1, na.rm = TRUE)
  Bias_NIE_BC1  <- Est_NIE_BC1 - true_value_BB[i, "TrueNIE"]
  Rbias100_NIE_BC1 <- 100 * (Est_NIE_BC1 - true_value_BB[i, "TrueNIE"]) / true_value_BB[i, "TrueNIE"]
  
  Est_NIE_BC2   <- mean(df$NIE_BC2, na.rm = TRUE)
  Var10_NIE_BC2 <- 10 * var(df$NIE_BC2, na.rm = TRUE)
  Bias_NIE_BC2  <- Est_NIE_BC2 - true_value_BB[i, "TrueNIE"]
  Rbias100_NIE_BC2 <- 100 * (Est_NIE_BC2 - true_value_BB[i, "TrueNIE"]) / true_value_BB[i, "TrueNIE"]
  
  #### MP
  Est_MP     <- mean(df$MP, na.rm = TRUE)
  Var10_MP   <- 10 * var(df$MP, na.rm = TRUE)
  Bias_MP    <- Est_MP - true_value_BB[i, "TrueMP"]
  Rbias100_MP <- 100 * (Est_MP - true_value_BB[i, "TrueMP"]) / true_value_BB[i, "TrueMP"]
  
  Est_MP_BC11   <- mean(df$MP_BC11, na.rm = TRUE)
  Var10_MP_BC11 <- 10 * var(df$MP_BC11, na.rm = TRUE)
  Bias_MP_BC11  <- Est_MP_BC11 - true_value_BB[i, "TrueMP"]
  Rbias100_MP_BC11 <- 100 * (Est_MP_BC11 - true_value_BB[i, "TrueMP"]) / true_value_BB[i, "TrueMP"]
  
  Est_MP_BC12   <- mean(df$MP_BC12, na.rm = TRUE)
  Var10_MP_BC12 <- 10 * var(df$MP_BC12, na.rm = TRUE)
  Bias_MP_BC12  <- Est_MP_BC12 - true_value_BB[i, "TrueMP"]
  Rbias100_MP_BC12 <- 100 * (Est_MP_BC12 - true_value_BB[i, "TrueMP"]) / true_value_BB[i, "TrueMP"]
  
  Est_MP_BC21   <- mean(df$MP_BC21, na.rm = TRUE)
  Var10_MP_BC21 <- 10 * var(df$MP_BC21, na.rm = TRUE)
  Bias_MP_BC21  <- Est_MP_BC21 - true_value_BB[i, "TrueMP"]
  Rbias100_MP_BC21 <- 100 * (Est_MP_BC21 - true_value_BB[i, "TrueMP"]) / true_value_BB[i, "TrueMP"]
  
  Est_MP_BC22   <- mean(df$MP_BC22, na.rm = TRUE)
  Var10_MP_BC22 <- 10 * var(df$MP_BC22, na.rm = TRUE)
  Bias_MP_BC22  <- Est_MP_BC22 - true_value_BB[i, "TrueMP"]
  Rbias100_MP_BC22 <- 100 * (Est_MP_BC22 - true_value_BB[i, "TrueMP"]) / true_value_BB[i, "TrueMP"]
  
  # Assemble summary rows
  result_NDE <- data.frame(
    Case = i, N = N,
    TrueNDE = t(true_value_BB[i, c(1)]),
    Est  = round(Est_NDE, 5),
...
names(BB_data_list)
final_list <- Simulation_summary_BB(
  BB_data_list[[1]],
  i = as.numeric(str_extract(names(BB_data_list)[1], "(?<=_i)\\d+")),
  N = as.numeric(str_extract(names(BB_data_list)[1], "(?<=_N)\\d+"))
)
for (i in 1:length(BB_data_list)) {
  temp_list <- Simulation_summary_BB(
    BB_data_list[[i]],
    i = as.numeric(str_extract(names(BB_data_list)[i], "(?<=_i)\\d+")),
    N = as.numeric(str_extract(names(BB_data_list)[i], "(?<=_N)\\d+"))
  )
...
# ------------------------------------------------------------
k <- 1
BB_data_list[[k]] <- BB_data_list[[k]] %>% filter(abs(MP_BC12) < 10 & abs(NDE_BC1) < 10)

k <- 2
BB_data_list[[k]] <- BB_data_list[[k]] %>% filter(abs(MP_BC12) < 10)

k <- 4
BB_data_list[[k]] <- BB_data_list[[k]] %>% filter(abs(MP_BC12) < 10 & abs(NDE_BC1) < 10)

k <- 7
BB_data_list[[k]] <- BB_data_list[[k]] %>% filter(abs(MP_BC12) < 10 & abs(MP_BC22) < 10 & abs(NDE_BC1) < 10)
